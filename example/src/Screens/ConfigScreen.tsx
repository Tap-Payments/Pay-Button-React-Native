...
function ConfigScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    setValue('buttonType', config.buttonType);
    ...
    setValue('powered', config.interface?.powered);

    setValue('authentication', config.transaction?.authentication);
    setValue('time', config.transaction?.authorize.time.toString());
    setValue('type', config.transaction?.authorize.type);
    setValue('paymentAgreement', config.transaction?.paymentAgreement.id);
    setValue('contract', config.transaction?.paymentAgreement.contract.id);
    setValue('reference', config.transaction?.reference);
  }, [
    config.buttonType,
    ...
    config.post?.url,
    config.scope,
    config.transaction?.authentication,
    config.transaction?.authorize.time,
    config.transaction?.authorize.type,
    config.transaction?.paymentAgreement.contract.id,
    config.transaction?.paymentAgreement.id,
    config.transaction?.reference,
    setValue,
  ]);
  ...
  const onSubmit: SubmitHandler<FieldValues> = (data) => {
    let newConfig: Config = {
      ...
      operator: { publicKey: data.key, hashString: '' },
      ...
      <ScrollView style={[styles.container]}>
        <View style={styles.container}>
          ...
          {renderList({
            ...
            isRequired: false,
            defaultOption: config.scope ?? '',
          })}
          {renderList({
            name: 'buttonType',
            title: 'buttonType',
            items: Object.keys(PayButtonType).map((item) => {
              return {
                key: item,
                value: item,
              };
            ...
            isRequired: true,
            defaultOption: config.buttonType,
          })}
          ...
          {renderSwitch('authentication', 'authentication')}
          {renderTextInput('time', 'time', false, 'numeric')}
          {renderList({
            name: 'type',
            title: 'type',
            items: Object.values(AuthorizeType).map((item) => ({
              key: item,
              value: item,
            })),
            isRequired: false,
            defaultOption: config.transaction?.authorize.type as string,
          })}
          {renderTextInput('paymentAgreement', 'paymentAgreement', false)}
          {renderTextInput('contract', 'contract', false)}
          {renderTextInput('reference', 'reference', false)}
